...
def skillfield(baseskill: Skill, fields: list[str]) -> SimpleNamespace:
    ...
    for field in fields:
        ...
        skill.name = f"{baseskill.name}:({field})"
        skill.field = field
        skills[field] = skill
    return SimpleNamespace(**skills)
class SKILLRANGES(Enum):
...
class Skill:
    ...
    level: int = SKILLRANGES.UNTRAINED.value
    field: str | None = None
    def __str__(self) -> str:
        """String representation of the Skill."""
        if self.field:
            return f"{self.name}:({self.field}) (Level: {SKILLRANGES(self.level).name}, Aptitude: {self.associated_aptitude.name})"
